...
ADDON=xbmcaddon.Addon(id='plugin.video.MG.Wizard')
...
def Fix_Special(url):
    dialogprocess.create("Kodi MG","Renaming paths...",'', 'Please Wait')
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("Kodi MG","Archiving...",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.video.MG.Wizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
    ...
    if os.path.exists(wtf_cache_path)==True:    
        for root, dirs, files in os.walk(wtf_cache_path):
            ...
            if file_count > 0:
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
	wtf_cache_path = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.video.exodus'), 'cache.db')
    if os.path.exists(wtf_cache_path)==True:    
        for root, dirs, files in os.walk(wtf_cache_path):
            file_count = 0
            file_count += len(files)
            if file_count > 0:
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
	wtf_cache_path = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.video.specto'), 'cache.db')
    if os.path.exists(wtf_cache_path)==True:    
        for root, dirs, files in os.walk(wtf_cache_path):
            file_count = 0
            file_count += len(files)
...
def Destroy_Path(path):
    dialogprocess.create("Kodi MG ","Cleaning...",'', 'Please Wait')
...
def Check_Path():
    if zip=='':
     if dialog.yesno("Kodi MG Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('Kodi MG Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("Kodi MG Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("Kodi MG Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("Kodi MG Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power","")	
def Backupzip():  
    if zip == '':
        dialog.ok('Kodi MG Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("Kodi MG Wizard","Backing Up",'', 'Please Wait')
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("Kodi MG Wizard", "You Are Now Backed Up", '','')
