...
import urllib
from exoscrapers.modules import client, getSum, cleantitle, source_utils, log_utils
	...
	def sources(self, url, hostDict, hostprDict):
		try:
			...
			for link in results:
				...
				link = link.replace('\r','').replace('\n','').replace('true','false').replace('%3A%2F%2F','://').replace('%2F','/')
				link = link.replace('https://www.faselhd.life/player/embed.php?url=','')					
				
				# if 'faselhd' in link:
					# #html = getSum.get(link, Type='cfscrape')
