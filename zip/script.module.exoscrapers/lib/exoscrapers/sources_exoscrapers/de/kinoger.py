...
from exoscrapers.modules import source_faultlog
from exoscrapers.modules.handler.requestHandler import cRequestHandler
            ...
            for container in link_containers:
                ...
                if ".show" in container.content:
                    ...
                    if source_link:
                        ...
                        if valid:
                            urlresolver = urlresolver.resolve(source_link)
                            if "quali" in urlresolver:
                                quality = re.findall('quali.*?=(.*?)\|', urlresolver)[0]
                    ...
                    if len(frame) == 0:
                        ...
                        urlresolver = urlresolver.resolve(frame[0].attrs["src"])
                        if "quali" in urlresolver:
                            quality = re.findall('quali.*?=(.*?)\|', urlresolver)[0]
