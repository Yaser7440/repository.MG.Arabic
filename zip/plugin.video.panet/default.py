	...
	for i in range(1,max):
		try:
			...
			for item in result.group(1).split('"panet-thumbnail"'):
				path ='http://www.82.80.40.135'+str(item).split('href="')[1].split('""><img')[0].strip()
				path = path[:-1]
	...
	for i in range(1, max):
		try:
			...
			for itr in result.group().split('panet-thumbnail"'):
				counter = counter+1
				if counter >1:
					path ='http://www.82.80.40.135'+str( itr).split('href="')[1].split('"><img')[0].strip()
...
def get_cats():
    req = urllib2.Request('http://82.80.40.135/series/')
    response = urllib2.urlopen(req)
    link = response.read()
    counter =0
    ...
    for item in result.group().split('href="'):
        counter = counter + 1
        if counter > 1:
			path='http://82.80.40.135'+str( item).split('">')[0].strip()
			path =path[:-1]
