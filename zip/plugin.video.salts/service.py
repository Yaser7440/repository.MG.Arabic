...
trakt_api = Trakt_API(TOKEN, kodi.get_setting('use_https') == 'true', int(kodi.get_setting('list_size')),
                      int(kodi.get_setting('trakt_timeout')), kodi.get_setting('trakt_offline') == 'true')
...
def show_next_up():
    global last_label
    global begin
    if xbmc.getInfoLabel('Container.PluginName') == kodi.get_id() and xbmc.getInfoLabel('Container.Content') == 'tvshows':
...
while not xbmc.abortRequested:
    ...
    xbmc.sleep(500)
    disable_global_cx()
    if TOKEN and kodi.get_setting('show_next_up') == 'true':
        show_next_up()
