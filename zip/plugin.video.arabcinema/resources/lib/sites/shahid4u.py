...
shahid4u = 'http://shahid4u.com/'

@utils.url_dispatcher.register('179', ['url'])
def Main(url):
    utils.addDir('[COLOR red]MG.Arabic[/COLOR] [COLOR white][B]Search[/B][/COLOR]',shahid4u + '?s=',174,'','')
    utils.addDir('[COLOR red]MG.Arabic[/COLOR] [COLOR white][B]TV SHOWS[/B][/COLOR]',shahid4u + '%D8%A7%D9%84%D9%85%D8%B3%D9%84%D8%B3%D9%84%D8%A7%D8%AA-%D8%A7%D9%84%D9%85%D9%83%D8%AA%D9%85%D9%84%D8%A9',178,'','')
    utils.addDir('[COLOR red]MG.Arabic[/COLOR] [COLOR white][B]MOVIES[/B][/COLOR]',shahid4u + 'category/movies/%D8%A7%D9%81%D9%84%D8%A7%D9%85-%D8%A7%D8%AC%D9%86%D8%A8%D9%89',171,'','')	
...
def List(url, page=1):
    ...
    for img, videopage, name, desc in match:
        ...
        utils.addDownLink('[B]%s[/B]'%name, videopage, 172, img, desc, '', 'N/A')
    try:
        nextp=re.compile('</a></li>\s*<li><a href="([^"]+)" >.*?&laquo;</a></li>\s*</ul></div>', re.DOTALL | re.IGNORECASE).findall(listhtml)[0]		
        utils.addDir('[COLOR red][B]Next Page[/B][/COLOR]', nextp, 171,'')
    except: pass
    xbmcplugin.endOfDirectory(utils.addon_handle)	
...
def TPList(url, page=1):
    ...
    try:
        ...
        utils.addDir('[COLOR red][B]Next Page[/B][/COLOR]', nextp, 178,'')
    except: pass
    xbmcplugin.endOfDirectory(utils.addon_handle)	
