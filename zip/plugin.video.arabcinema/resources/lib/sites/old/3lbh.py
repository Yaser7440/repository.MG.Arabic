...
def TPMain():
    utils.addDir('[COLOR red][B]MENU[/B][/COLOR]',Albh_url,99,'','')
    utils.addDir('[COLOR red][B]GENRES[/B][/COLOR]',Albh_url + '/movies',95,'','')
    utils.addDir('[COLOR red][B]MOVIES[/B][/COLOR]',Albh_url + '/movies',91,'','')	
    utils.addDir('[COLOR red][B]TV SHOWS[/B][/COLOR]',Albh_url + '/eseries',91,'','')
    utils.addDir('[COLOR red][B]ARABIC TV SHOWS[/B][/COLOR]',Albh_url + '/aseries',97,'','')
...
def menu(url):
    ...
    for caturl, menu in match:
        ...
        if '/aseries' in caturl:
           utils.addDir('[B]%s[/B]'%decodename(menu), caturl, 97, '', '', '')
        else:  
           utils.addDir('[B]%s[/B]'%decodename(menu), caturl, 91, '', '', '')
    xbmcplugin.endOfDirectory(utils.addon_handle)	
...
def TPList(url, page=1):
    ...
    for videourl, thumb, name, rating, desc in match:
        ...
        if '/eseries' in url:
		    utils.addDir('[B]%s[/B]'%decodename(name), videourl, 93, thumb, utils.cleantext(desc), '', rating)
        else:		
		    utils.addDownLink('[B]%s[/B]'%decodename(name), videourl, 92, thumb, utils.cleantext(desc), '',rating)
    try:
		nextp=re.compile('<li><a href="(.*?)" rel="next">&raquo;</a></li>').findall(listhtml)[0]
		utils.addDir('Next Page ' ,nextp,  91, '')
    except: pass
    xbmcplugin.endOfDirectory(utils.addon_handle)
...
def TPList2(url, page=1):
    ...
    for videourl, thumb, name, desc in match:
        ...
        name = utils.cleantext(name)
        videourl = Albh_url + videourl
        utils.addDir('[B]%s[/B]'%decodename(name), videourl, 96, thumb, utils.cleantext(desc), '','')
    try:
		nextp=re.compile('<li><a href="(.*?)" rel="next">&raquo;</a></li>').findall(listhtml)[0]
		utils.addDir('Next Page ' ,nextp,  97, '')
    except: pass
    xbmcplugin.endOfDirectory(utils.addon_handle)
...
def SEASON(url):
    ...
    for caturl, thumb, season in match:
        caturl = Albh_url + caturl 
        thumb = Albh_url + thumb 		
        utils.addDir('[B]%s[/B]'%decodename(season), caturl, 96, thumb, '', '')
    xbmcplugin.endOfDirectory(utils.addon_handle)	
...
def episode(url):
    ...
    for caturl, episode in match:
        caturl = Albh_url + caturl
        if '/aseries' in caturl: 		
            utils.addDownLink('[B]%s[/B]'%utils.cleantext(episode), caturl, 98, thumb, '','')
        else:
...
def TPPlayvid(url, name, download=None):
    ...
    for name in match1:
        name = utils.cleantext(name)
    videopage = utils.getHtml(url, '')
    match = re.compile('<video.*?src="([^"]+)"', re.DOTALL | re.IGNORECASE).findall(videopage)
    if match:
        videourl = match[0]
