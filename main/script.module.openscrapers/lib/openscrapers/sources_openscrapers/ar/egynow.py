...
from openscrapers.modules import client, cleantitle, log_utils, source_utils, jsunpack
							...
							if 'moshahda' in video:
								try:
									...
									for link, label in jc:
										...
										link += '|Referer=%s&User-Agent=%s' % (urllib.quote(client.agent()), video)
										sources.append({'source': 'moshahda', 'quality': quality, 'language': 'ar', 'url': link, 'info': info,
														'direct': True, 'debridonly': False})
								except Exception:
									pass									
							elif 'uppom' in video:
