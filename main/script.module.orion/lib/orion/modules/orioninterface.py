...
class OrionInterface:
	...
	@classmethod
	def dialogVisible(self, id = None, loader = False):
		current = self.dialogId()
		if current and current <= OrionInterface.IdNone: current = None
		elif not loader and (current == OrionInterface.IdBusyDialog or current == OrionInterface.IdBusyDialogNoCancel): current = None
